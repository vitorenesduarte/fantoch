    ...
    V: Clone,
{
    ...
    /// Skips prepare phase and generates the first ballot (panics if not the first).
    /// This ballot can be safely used (without a prepare phase) in the slow path by the original
    /// coordinator because any other ballot will be higher than n, and thus, it's not possible that
    ...
    /// Paxos agent (i.e. either the proposer or the acceptor).
    pub fn handle(&mut self, from: ProcessId, msg: SynodMessage<V>) -> Option<SynodMessage<V>> {
        match msg {
            // once this is handled, and the acceptor's value is blindly changed, any message to be
            // handled by the acceptor must only be forwarded if the value has not been chosen
            ...
            SynodMessage::MPrepare(b) => self.chosen().or_else(|| self.acceptor.handle_prepare(b)),
            ...
            SynodMessage::MAccepted(b) => self.proposer.handle_accepted(from, b, &self.acceptor),
        }
    }
        ...
        if self.ballot == b {
            ...
            if self.accepts.len() == self.f + 1 {
                ...
                let proposal = if let Some(proposal) = proposal {
                    ...
                    // otherwise, check if we're still at the first ballot (that doesn't need to be
                    // prepared)
    ...
    V: Clone,
{
    ...
    // Set the consensus value when it's chosen. This API shouldn't be exposed publicly and should
    // be used with care.
